...
extern unsigned int mldsa_pub_key_len;
#endif
#endif
 ...
 *
 * In hybrid mode (a classical algorithm together with ML-DSA), both keys
 * are embedded and bootutil_keys[] holds two entries; bootutil_find_key()
 * matches each signature's KEYHASH/PUBKEY TLV against whichever entry has
 * the matching key bytes, regardless of algorithm.
 */
...
#if defined(MCUBOOT_SIGN_MLDSA) && \
    (defined(MCUBOOT_SIGN_RSA) || defined(MCUBOOT_SIGN_EC256) || defined(MCUBOOT_SIGN_ED25519))
